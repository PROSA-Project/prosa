...
def create_db(opts, all_proofs):
    ...
    for fname, line, claim, length in all_proofs:
        if fname in db["manual_exceptions"] and claim in db["manual_exceptions"][fname]:
...
def check_proof_lengths(opts, all_proofs):
    ...
    for fname, line, claim, length in all_proofs:
        ...
        for category in ["manual_exceptions", "legacy_proofs"]:
            if (
                fname in opts.long_proofs[category]
                and claim in opts.long_proofs[category][fname]
                and opts.long_proofs[category][fname][claim] >= length
            ):
        ...
        if not known:
            print(
                "Warning: new long proof of %s in %s:%d!" % (claim, fname, line),
                file=sys.stderr,
            )
    ...
    if known_long_proofs:
        ...
        print(
            "Checked %d proofs in %d files." % (len(all_proofs), len(opts.input_files))
        )
...
def parse_args():
    ...
    parser.add_argument(
        ...
        default=False,
        action="store_true",
        help="complain and return non-zero exit code if there are new too-long proofs",
    )
